  ...
  return (
    <div>
      ...
      {truncatedBlogs.map((blog) => (
        <div>
          <Link href={`${linkResolver(blog.youtube,blog.youtubeURL,blog.slug)}`}><Image className="w-96 lg:w-full m-1 border-opacity-0 hover:border-opacity-100 hover:scale-95 border-2 xl:border-4 border-tgs-purple rounded-md" src={`${eventImage(blog)}`} alt={`${blog.name} Cover`} width={300} height={300}></Image></Link>
        </div>
      ))}
