...
export async function generateMetadata({
  ...
  const imageUrl =
    urlFor(post.thumb)?.width(1000).height(1000)?.url() ||
    "https://cdn.sanity.io/images/fnvy29id/tgs/4522c0ed77190763f1d85445d4cd67d4b9f7fb7c-1080x1080.png?w=1000&h=1000";
...
export default async function Page({
  ...
  const prevPosts = await sanityFetch<SanityDocument[]>({
    query: PREV_SLUG_QUERY,
  });
  ...
  const nextPosts = await sanityFetch<SanityDocument[]>({
    query: NEXT_SLUG_QUERY,
  });
  ...
  return (
    <div className="font-roc text-lg text-balance max-md:mt-14 max-[300px]:w-80 text-white">
      <div className="m-5 ">
        <div className="flex items-center justify-between mb-1">
          <div className="xl:text-2xl text-lg font-bit">
            <Link
              className="hover:underline max-md:text-6xl"
              href={"/feature/2024"}
            >
              &lt;<span className="max-md:hidden"> Top 50 Albums</span>
            </Link>
          </div>
          <div className="text-4xl font-bold font-title flex-1 text-center">
            {" "}
            {/* Title */}
            {post.name}
          </div>
        ...
        <div className="text-xl md:text-2xl font-bold font-bit text-center">
          {" "}
          {/* Title */}
          {post.artist}
        </div>
      ...
      <div className="flex justify-center">
        <Link
          href={`/feature/2024/${prevPostslug}`}
          className="mx-2 md:mx-10 self-center justify-items-start font-title text-5xl "
        >
          &lt;
          <div className="text-sm font-bit hidden text-center">
            {prevPost.artist}
          </div>
        </Link>
        ...
        <Link
          href={`/feature/2024/${nextPostslug}`}
          className="mx-2 md:mx-10 self-center justify-items-end font-title text-5xl "
        >
          &gt;
          <div className="text-sm font-bit hidden text-center place-self-end">
            {nextPost.artist}
          </div>
        </Link>
      </div>
      <div className="text-md font-bold font-title text-center -mb-8 mt-2 md:mt-4">
        {" "}
        {/* Title */}
      ...
      <div className="md:mx-14 mx-8 text-lg lg:text-2xl text-pretty text-justify pt-10 indent-8">
        ...
        <div className=" xl:text-2xl text-lg font-bit text-right">
          {" "}
          {/* Title */}
          <div>-{post.writer}</div>
        </div>
